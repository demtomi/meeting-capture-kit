...
`manifest.json` carries `schema` (now `2`), `meeting_id`, `label`, `source`, `started_at`, `shared_start_monotonic_ns`, `tracks`, and `output_dir`. It adds `language_hint` when you pass `--lang`, and `expected_speakers` when you pass `--speakers`, on any source. On `mic+system` that number is the **remote** head count, on `mic-multi` the head count in the room, and absent means not known. Schema 1 wrote `expected_speakers` only for `mic-multi`. Each entry in `tracks` names a file relative to the work directory and says whether it is the host track.
